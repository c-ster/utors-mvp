...
import { SlatingPage } from '@/features/slating/SlatingPage';
import { RosterPage } from '@/features/roster/RosterPage';
import { IntakePage } from '@/features/intake/IntakePage';
import { WargamePage } from '@/features/wargame/WargamePage';
import { cn } from '@/lib/utils';
import {
  LayoutDashboard, Wand2, Users, ClipboardList, Swords,
...
type Page = 'dashboard' | 'slating' | 'roster' | 'intake' | 'wargame';

const NAV_ITEMS: { key: Page; label: string; icon: React.ElementType }[] = [
  { key: 'dashboard', label: "Commander's Dashboard", icon: LayoutDashboard },
  { key: 'slating', label: 'Slate Generator', icon: Wand2 },
  { key: 'roster', label: 'Unit Roster', icon: Users },
...
function AppContent() {
  ...
  const [currentUic] = useState(DEFAULT_UIC);
  ...
  return (
    <div className="flex h-screen bg-slate-950">
      {/* Sidebar */}
      <aside className="w-56 bg-slate-900/80 border-r border-slate-700/50 flex flex-col">
        {/* Logo */}
      ...
      <main className="flex-1 overflow-auto">
        {currentPage === 'dashboard' && <DashboardPage uic={currentUic} />}
        {currentPage === 'slating' && <SlatingPage uic={currentUic} />}
        {currentPage === 'roster' && <RosterPage uic={currentUic} />}
        {currentPage === 'intake' && <IntakePage />}
        {currentPage === 'wargame' && <WargamePage uic={currentUic} />}
      </main>
    </div>
  );
}
